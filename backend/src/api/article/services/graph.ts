...
interface Edge {
  id: number;
  sourceArticle: { id: number };
  targetArticle: { id: number };
  weight: number;
  isActive: boolean;
}

interface GraphData {
  nodes: Map<number, Article>;
  edges: Edge[];
  adjacencyList: Map<number, number[]>;
  inLinks: Map<number, number[]>;
  outLinks: Map<number, number[]>;
}
...
export default () => ({
   ...
   */
  async calculatePageRank(iterations = 20, dampingFactor = 0.85): Promise<void> {
   ...
   */
  async calculateHITS(iterations = 20): Promise<void> {
    ...
    // Iterate
    for (let iter = 0; iter < iterations; iter++) {
      const newAuthority = new Map<number, number>();
      ...
      for (const score of newAuthority.values()) authNorm += score * score;
   ...
   */
  async updateDegreeCounts(): Promise<void> {
   ...
   */
  async getRecommendations(articleId: number, limit = 5): Promise<Article[]> {
  ...
  async buildGraphData(): Promise<GraphData> {
    ...
    const nodes = new Map<number, Article>();
    ...
    for (const edge of edges) {
      const sourceId = edge.sourceArticle?.id;
      const targetId = edge.targetArticle?.id;
   ...
   */
  async calculateCommunities(): Promise<Map<number, number>> {
   ...
   */
  async calculateBetweennessCentrality(): Promise<void> {
   ...
   */
  async findShortestPath(sourceId: number, targetId: number): Promise<number[] | null> {
    const graph = await this.buildGraphData();
   ...
   */
  async getSubgraph(articleId: number, depth = 2) {
    const graph = await this.buildGraphData();
    ...
    const subgraphNodes = new Set<number>([articleId]);
    const currentLevel = new Set<number>([articleId]);
   ...
   */
  async getBridgeArticles(limit = 10): Promise<Article[]> {
    const articles = await strapi.db.query('api::article.article').findMany({
      where: {
   ...
   */
  async getCommunityMembers(articleId: number, limit = 20): Promise<Article[]> {
   ...
   */
  async getStructurallySimilar(articleId: number, limit = 5): Promise<Article[]> {
